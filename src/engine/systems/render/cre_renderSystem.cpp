...
void renderSystem_ProcessCommands(EntityRegistry *reg, CommandBus *bus) {
  ...
  while (CommandBus_Next(bus, &iter, &cmd)) {
    if ((cmd->type & CMD_DOMAIN_MASK) != CMD_DOMAIN_RENDER)
      continue;
    ...
    case CMD_RENDER_SETDEPTHMATH: {
      renderSystem_SetDepthMath(
          cmd->renderDepth.wX, cmd->renderDepth.wY, cmd->renderDepth.wH,
          cmd->renderDepth.shiftBatch, cmd->renderDepth.shiftDepth);
      continue;
    }
    default:
      break;
...
static void _renderSystem_InitBatchTable(void) {
  if (s_batchTableInitialized)
    return;

  memset(render_state_table, 0, sizeof(render_state_table));

  // You can add this as an parameter. No need to link assetManager for one
  // line.
