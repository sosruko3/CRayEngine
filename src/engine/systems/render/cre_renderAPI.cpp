...
void renderAPI_SetDepthSettings(CommandBus &bus, float wX, float wY, float wH,
  ...
  Command cmd = {.type = CMD_RENDER_SETDEPTHMATH,
...
void renderAPI_SetSprite(CommandBus &bus, Entity entity, uint16_t spriteID) {
  Command cmd = {.type = CMD_RENDER_SET_SPRITE,
                 .entity = entity,
                 .u16 = {.value = spriteID}};
  CommandBus_Push(bus, cmd);
}
...
void renderAPI_SetColor(CommandBus &bus, Entity entity, creColor color) {
  Command cmd = {.type = CMD_RENDER_SET_COLOR,
                 .entity = entity,
                 .color = {.color = color}};
  CommandBus_Push(bus, cmd);
}
...
void renderAPI_SetVisible(CommandBus &bus, Entity entity, bool visible) {
  Command cmd = {.type = CMD_RENDER_SET_VISIBLE,
                 .entity = entity,
                 .b8 = {.value = visible}};
  CommandBus_Push(bus, cmd);
}
...
void renderAPI_SetVisualScale(CommandBus &bus, Entity entity, float scaleX,
                              float scaleY) {
  Command cmd = {.type = CMD_RENDER_SET_VISUAL_SCALE,
                 .entity = entity,
                 .vec2 = {.value = creVec2{scaleX, scaleY}}};
  CommandBus_Push(bus, cmd);
}

void renderAPI_SetRotation(CommandBus &bus, Entity entity, float rotation) {
  Command cmd = {.type = CMD_RENDER_SET_ROTATION,
                 .entity = entity,
                 .f32 = {.value = rotation}};
  CommandBus_Push(bus, cmd);
}
...
void renderAPI_SetRenderLayer(CommandBus &bus, Entity entity, uint8_t layer) {
  Command cmd = {
      .type = CMD_RENDER_SET_LAYER, .entity = entity, .u8 = {.value = layer}};
  CommandBus_Push(bus, cmd);
}
