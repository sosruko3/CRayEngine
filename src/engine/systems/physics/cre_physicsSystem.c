    ...
    static PhysMaterial g_materials[PHYS_MAX_MATERIALS] = {
        [MAT_DEFAULT] = { .density = 2.0f,  .friction = 0.5f,  .restitution = 0.2f },
        ...
        [MAT_PLAYER]  = { .density = 1.0f,  .friction = 0.5f,  .restitution = 0.2f},
    ...
    static bool CheckCircle_Circle(
        ...
        float* out_overlap, creVec2* out_normal);
    ...
    static void Phase3_DetectContacts(EntityRegistry* reg) {
        ...
        for (uint32_t i = 0; i <= bound; i++) {
            ...
            // Filter: Active, has physics, not static, not sleeping, This part is kinda confusing.
        ...
        float overlap, creVec2 normal)
    {
        ...
        float jt = -(relVelX2 * tangentNormX + relVelY2 * tangentNormY) * EffectiveMassSum;
