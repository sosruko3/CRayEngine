 ...
 * @return true if already seen this query, false if first time (and marks as
 * seen)
 */
...
static inline int GetHashIndex(int cellX, int cellY) {
  unsigned int h1 = static_cast<unsigned int>(cellX) * 73856093;
  unsigned int h2 = static_cast<unsigned int>(cellY) * 19349663;
  return static_cast<int>((h1 ^ h2) & SPATIAL_HASH_MASK); // Bitmask, not modulo
}
...
void SpatialHash_AddStatic(uint32_t entityID, int x, int y, int width,
                           int height) {
  int32_t minX = WORLD_TO_GRID(x);
  int32_t minY = WORLD_TO_GRID(y);
  int32_t maxX = WORLD_TO_GRID(x + width);
  int32_t maxY = WORLD_TO_GRID(y + height);
  ...
  for (int32_t cy = minY; cy <= maxY; cy++) {
    for (int32_t cx = minX; cx <= maxX; cx++) {
      ...
      if (nodeIdx == SPATIAL_NULL_IDX)
        return;

      staticNodes[nodeIdx].entityID = entityID;
      staticNodes[nodeIdx].gridX = cx;
      staticNodes[nodeIdx].gridY = cy;
...
void SpatialHash_AddDynamic(uint32_t entityID, int x, int y, int width,
  ...
  for (int32_t cy = minY; cy <= maxY; cy++) {
    ...
    const unsigned int hy = static_cast<unsigned int>(cy) * 19349663;

    for (int32_t cx = minX; cx <= maxX; cx++) {
      // Inline hash calculation (avoids function call overhead)
      const unsigned int hx = static_cast<unsigned int>(cx) * 73856093;
      const int bucketIndex = static_cast<int>(((hx ^ hy) & SPATIAL_HASH_MASK));
      ...
      dynamicNodes[nodeIdx].gridX = cx;
      dynamicNodes[nodeIdx].gridY = cy;
  ...
  for (int cy = minY; cy <= maxY; cy++) {
    for (int cx = minX; cx <= maxX; cx++) {
      ...
      while (curr != SPATIAL_NULL_IDX) {
        ...
        if (node->entityID == entityID && node->gridX == cx &&
            node->gridY == cy) {
          // Unlink from list
