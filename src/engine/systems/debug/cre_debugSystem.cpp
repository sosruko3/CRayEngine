...
static const char *s_modeNames[] = {"OFF", "Spatial Hash Heatmap",
                                    "Entity State Overlay", "Velocity Field",
                                    "Collision Layers"};
...
void DebugSystem_Init(void) {
  ...
  Log(LOG_LVL_INFO,
      "Debug System Initialized - Press F1 to toggle, F2-F5 for modes");
}
...
void DebugSystem_HandleInput(EntityRegistry *reg) {
  ...
  if (IsKeyPressed(KEY_F3)) {
    s_currentMode = DEBUG_MODE_ENTITY_STATE;
    s_debugEnabled = true;
  }
  if (IsKeyPressed(KEY_F4)) {
    s_currentMode = DEBUG_MODE_VELOCITY_FIELD;
    s_debugEnabled = true;
  }
  if (IsKeyPressed(KEY_F5)) {
    s_currentMode = DEBUG_MODE_COLLISION_LAYERS;
    s_debugEnabled = true;
  }
  ...
  if (IsKeyPressed(KEY_F8)) {
    s_currentMode =
        (DebugVisualizationMode)((s_currentMode + 1) % DEBUG_MODE_COUNT);
    if (s_currentMode == DEBUG_MODE_OFF)
      s_currentMode = DEBUG_MODE_SPATIAL_HASH;
    s_debugEnabled = true;
  }
...
static const CameraComponent *
DebugSystem_GetActiveCameraComponent(const EntityRegistry *reg) {
  if (!reg)
    return NULL;
  int32_t camIndex = cameraSystem_FindActive(reg);
  if (camIndex < 0 || camIndex >= (int32_t)MAX_CAMERAS)
    return NULL;
...
void DebugSystem_RenderMouseHover(EntityRegistry *reg) {
  ...
  if (flags & FLAG_ACTIVE)
    flagsLen +=
        snprintf(flagsStr + flagsLen, sizeof(flagsStr) - flagsLen, "ACTIVE ");
  if (flags & FLAG_VISIBLE)
    flagsLen +=
        snprintf(flagsStr + flagsLen, sizeof(flagsStr) - flagsLen, "VISIBLE ");
  if (flags & FLAG_STATIC)
    flagsLen +=
        snprintf(flagsStr + flagsLen, sizeof(flagsStr) - flagsLen, "STATIC ");
  if (flags & FLAG_SLEEPING)
    flagsLen +=
        snprintf(flagsStr + flagsLen, sizeof(flagsStr) - flagsLen, "SLEEPING ");
  if (flags & FLAG_CULLED)
    flagsLen +=
        snprintf(flagsStr + flagsLen, sizeof(flagsStr) - flagsLen, "CULLED ");
  if (flags & FLAG_ALWAYS_AWAKE)
    flagsLen += snprintf(flagsStr + flagsLen, sizeof(flagsStr) - flagsLen,
                         "ALWAYS_AWAKE ");
...
static void RenderEntityStateOverlay(EntityRegistry *reg) {
  ...
  for (uint32_t i = 0; i < bound; i++) {
    ...
    if (isnan(px) || isnan(py) || isinf(px) || isinf(py)) {
      ...
      DrawText(nanStr, (int)(camPos.x - 60), (int)(camPos.y - 10), 16,
               R_COL(colorNaN));
    ...
    if (px < -ORPHAN_THRESHOLD || px > ORPHAN_THRESHOLD ||
      ...
      if (dist > 1.0f) {
        ...
        DrawLine((int)edgeX, (int)edgeY, (int)(edgeX + nx * 20),
                 (int)(edgeY + ny * 20), R_COL(colorOrphan));
      ...
      continue; // Don't draw normal overlay for orphans (they're off-screen
                // anyway)
    }
...
static void RenderCollisionLayers(EntityRegistry *reg) {
  ...
  for (uint32_t i = 0; i < bound; i++) {
    ...
    if (hasAabb) {
      DrawRectangleLines((int)(drawX - 1), (int)(drawY - 1), (int)w + 2,
                         (int)h + 2, R_COL(layerColor));
    } else if (hasCircle) {
      float radius = w * 0.5f;
      if (radius > 0.0f) {
        DrawCircleLines((int)(drawX + radius), (int)(drawY + radius), radius,
                        R_COL(layerColor));
      }
    }
...
static void RenderLegendEntityState(void) {
  ...
  creColor stateColors[] = {{0, 228, 48, 255},
                            {255, 161, 0, 255},
                            {230, 41, 55, 255},
                            {100, 100, 100, 255}};
