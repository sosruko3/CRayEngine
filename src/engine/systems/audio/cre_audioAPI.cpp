...
void audioAPI_PlayOneShot(CommandBus *restrict bus, AudioGroupID groupID, AudioSourceID sourceID) {
    ...
    Command cmd = {
        ...
        .entity = ENTITY_INVALID,
        .audioshot = { 
            .sourceid = (uint16_t)sourceID,
            .groupid = (uint8_t)groupID,
         },
    };
...
void audioAPI_SoundSetSpatialization(CommandBus *restrict bus, AudioID id, bool enabled) {
    ...
    if (!CommandBus_Push(bus, cmd)) {
        Log(LOG_LVL_WARNING, "audioAPI_SoundSetSpatialization: CommandBus is full!");
    }
}
